...
> 6. Chạy local bằng `npm run dev` (cổng `5100`) với `.env.local` trỏ tới Workspace test; trang tự chuyển tới đăng nhập Cogover và quay lại localhost, người dùng tự đăng nhập trên trình duyệt, agent không nhập credential. Ở standalone `useAppSlug()` trả chuỗi rỗng; mọi điều hướng nội bộ dùng `Link` của template.
